...
export function RevealText({
  ...
  fontSize = "text-[80px] md:text-[120px] lg:text-[150px]",
  letterDelay = 0.08,
  overlayDelay = 0.05,
  overlayDuration = 0.4,
  springDuration = 600,
      ...
      <div className="flex flex-wrap justify-center">
        {text.split("").map((letter, index) => (
          <motion.span
            ...
            className={`${fontSize} font-black tracking-tight cursor-pointer relative overflow-hidden inline-block px-1`}
            initial={{ 
              scale: 0,
              opacity: 0,
            }}
          ...
          >
            ...
            {showOverlayText && (
              <motion.span
                className={`absolute inset-0 flex items-center justify-center ${overlayColor} pointer-events-none`}
                initial={{ opacity: 0 }}
                animate={{ 
                  opacity: [0, 1, 1, 0]
                }}
